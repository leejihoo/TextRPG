...
int InputInt()
{
	...
	if (cin.fail())
	{
		cin.clear();
		cin.ignore(1024, '\n');
		return INT_MAX; // INT_MAX가 return되면 압력이 잘못되었음을 의미
	}
...
_tagMonster CreateMonster(const char* pName, int iAttakMin,
...
void OutputPlayer(_tagPlayer* pPlayer)
{
	...
	/*cout << "공격력 : " << pPlayer->iAttackMin << " - " <<
		pPlayer->iAttackMax << "\t방어력 : " << pPlayer->iArmorMin <<
		" - " << pPlayer->iAttackMax << endl;*/
	...
	cout << "체력 : " << pPlayer->iHP << " / " << pPlayer->iHPMax <<
		"\t마나 : " << pPlayer->iMP << " / " << pPlayer->iMPMax << endl << endl;
...
{
	/*int iAttack = rand() % (pPlayer->iAttackMax - pPlayer->iAttackMin) + (pPlayer->iAttackMin);
						int iArmor = rand() % (pMonster->iArmorMax - pMonster->iArmorMin) + (pMonster->iArmorMin);*/
	...
	iAttack = rand() % (pMonster->iAttackMax - pMonster->iAttackMin) + (pMonster->iAttackMin);
	/* iArmor = rand() % (pPlayer->iArmorMax - pPlayer->iArmorMin) + (pPlayer->iArmorMin);*/
